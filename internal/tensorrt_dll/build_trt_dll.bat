...
:: Find MSVC cl.exe (try with VC requirement, fallback to any VS)
set "VS_PATH="
...
if not defined VS_PATH (
  for /f "usebackq tokens=*" %%i in (`"%ProgramFiles(x86)%\Microsoft Visual Studio\Installer\vswhere.exe" -latest -all -products * -property installationPath 2^>nul`) do (
    set "VS_PATH=%%i"
  )
)
...
cl.exe /LD /EHsc /std:c++17 /O2 /wd4819 ^
